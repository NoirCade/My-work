...
# create DQN agent
model = DQN('CnnPolicy', env, learning_rate=1e-4, buffer_size=100000, batch_size=64, learning_starts=10000,
            exploration_fraction=0.3, exploration_initial_eps=1.0, exploration_final_eps=0.01, train_freq=4,
            target_update_interval=5000, gamma=0.99, verbose=1, device=device)
...
# model = PPO('CnnPolicy', env, verbose=1, batch_size=64, gamma=0.99,
#             learning_rate=2.5e-4, ent_coef=0.01, vf_coef=0.5, clip_range=0.2)

# load the model
# model = DQN.load('DQN_v4_200000.zip', env=env, device=device)    # DQN model
